...
# Wide to long
coup_rel_l <- melt(coup_rel, id.vars = "PID")
...
coup_rel_sum <- coup_rel_l[, .(value = mean(value + 1, na.rm = T)), by = .(PID, scale)]
coup_rel_sum <- dcast(coup_rel_sum, PID ~ scale)

ggplot(coup_rel_sum, aes(x = Relevance, y = Attitude)) +
