...
plot_rating_ID <- function(model,
  ...
  # Prepare values for participant-wise prediciton ----
  plot_dat_Ps <- unique(as.data.table(model$data)[, c("PID", "block", measures), with = F])
  
  # Compute quadratics
  if (!is.null(quadratic)) {
    lapply(quadratic, function(x) {
      plot_dat_Ps[, paste0("I(", x, ")^2") := .(get(x) ^ 2)]
    })
  }
  ...
  plot_pred <- function(resp,
    ...
    # Predict - this returns ndraws x npoints x 5
    print("Predicting participant-wise effect")
    pred_Ps <- fitted(
      model,
      newdata = plot_dat_Ps,
      re_formula = ~ (1 + block | PID),
      summary = F,
      resp = resp
    )
    
    # Summarize into mean raiting
    pred_Ps <- sum_ordinal(pred_Ps)
    
    # Combine with predictors
    pred_Ps <- cbind(plot_dat_Ps, pred_Ps)
    
    # Bin and summarize
    pred_Ps[, bin_n := factor(dplyr::ntile(get(col), nbins))]
    pred_Ps[, bin := mean(get(col)), by = bin_n]
    pred_Ps[, prec := 1 / sd]
    pred_Ps_sum <-
      pred_Ps[, .(m = wtd.mean(m, weights = prec, normwt = T),
                  se = sqrt(wtd.var(
                    m, weights = prec, normwt = T
                  )
                  / .N)),
              by = c("bin", "block")]
    pred_Ps_sum[, block := factor(block,
                                  levels = block_levels,
                                  labels = block_labels)]
